...
## Iteration 2026-04-23 - Price delta anomaly filtering

### Что truthfully сделано

- Для price-delta по PDF введена базовая фильтрация аномалий:
  - слишком резкие скачки (`>= 40%`) больше не идут в основной блок `Изменение цен`;
  - такие случаи теперь уходят в `Проверить вручную`.
- Выполнен manual-run:
  - `python3 PACK-warehouse/tools/warehouse_reports_pipeline.py --hours 720 --manual`

### Результат ручной проверки

- Из основного блока `Изменение цен` ушёл шумный кейс:
  - `Смакотерия: Сосиска в тесте (180.00 -> 47.17) (-73.8%)`
- Этот кейс теперь виден в `Проверить вручную` как:
  - `Аномалия price delta по накладным: ...`

### Truthful verdict

- Price-layer стал аккуратнее и ближе к manager-ready формату.
- Но это всё ещё не финальная версия:
...
## Iteration 2026-04-23 - ABC manager signals

### Что truthfully сделано

- `ABC`-слой перестал быть только источником букв `A/B/C`.
- Из `ABC`-файлов теперь извлекаются:
  - выручка
  - количество
  - цена
  - себестоимость
  - сумма себестоимости
  - доля себестоимости
- В manager-report появился отдельный блок `ABC и себестоимость`.

### Ручная проверка

- Выполнен manual-run:
  - `python3 PACK-warehouse/tools/warehouse_reports_pipeline.py --hours 720 --manual`
- После дополнительной фильтрации leaf-строк в блоке больше нет верхнеуровневых агрегатов типа `НАПИТКИ`, `Кофе`, `Еда`.
- В актуальном отчёте появились manager-ready сигналы, например:
  - лидеры `ABC`: `Капучино`, `Большой Капучино`, `Латте`, `Американо`, `Десерт 350`
  - тяжёлые по себестоимости позиции
  - `C`-позиции под пересмотр

### Truthful verdict

- Это уже реальный шаг от `ABC-файла` к управленческому смыслу.
- Но cost-layer ещё не финален:
  - есть спорные позиции вроде `персонал`, которые требуют отдельной фильтрации или правила исключения;
  - `cost / margin cards` как отдельный слой ещё не materialized полностью.
...
## Iteration 2026-04-23 - ABC live document-chain walkthrough

### Что truthfully сделано

- Materialized первый живой walkthrough по типу документа:
  - `WH.WP.008-abc-document-chain-live-walkthrough-2026-04-23.md`
- В walkthrough зафиксирована реальная цепочка:
  - какие поля кладовщик извлекает из `ABC`;
  - какие derived metrics уже считает;
  - какие блоки реально попадают руководителю в `WH.REPORT.002`;
  - какие хвосты ещё мешают считать `ABC` полностью доведённым.

### Что это дало

- `ABC` теперь formalized не только как skill и метод, но и как живая предметная карта на фактических файлах.
- Следующий слой по `ABC` narrowed:
  - нужно отделить полноценные SKU от модификаторов/полустрок;
  - потом связать `ABC` с `каталогом` и `накладными` в `unit economics`.

### Truthful next step

Следующий bounded slice по `ABC`:

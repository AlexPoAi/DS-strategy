...
## Прогресс 2026-04-21 — factual finding по ABC intake
...
- После live-inspection исходных `xlsx` выяснилось, что оба `ABC`-файла содержат только титульную шапку отчёта (`ABC и XYZ анализ`, период, раздел каталога, настройки групп).
- Внутри этих двух файлов нет табличной части с позициями и колонками `SKU / выручка / qty / A-B-C`.
- Значит, текущий blocker уже не в кладовщике и не в intake, а в самом формате входного `ABC`-экспорта.

### Следующий правильный шаг

1. Получить от Жанны / источника полноценный `ABC`-экспорт с таблицей позиций, а не header-only файл.
2. После появления корректного файла повторно прогнать intake и только тогда валидировать `A/B/C -> decision layer`.
3. Параллельно продолжать `PDF invoice -> line items` слой, потому что он не зависит от дефекта текущего `ABC`-экспорта.

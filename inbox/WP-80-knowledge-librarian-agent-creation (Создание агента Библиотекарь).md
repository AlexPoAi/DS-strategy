...
Рабочее русское имя агента согласовано с пользователем: `Библиотекарь`.

# Цель

Создать в агентстве нового внутреннего агента `Knowledge Registry Curator — Библиотекарь` и встроить его в реестр агентства как штатную роль.

# Scope

1. Описать контракт агента.
2. Зафиксировать его место в контуре:
   - `Extractor -> Библиотекарь -> Strategist`
...
Создан новый внутренний агент `Knowledge Registry Curator — Библиотекарь`.

Зафиксировано:
- его место в контуре `Extractor -> Библиотекарь -> Strategist`;
...
2. Открыть отдельный slice на `skill` для Библиотекаря.

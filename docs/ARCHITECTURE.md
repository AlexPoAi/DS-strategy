...
~/Documents/
└── Творческий конвеер/        ← Obsidian vault / человеческий мозг системы
```
...
| `Творческий конвеер` | Human brain | Локальный Obsidian-vault: заметки, сессии стратегирования, human-facing навигация |
...
[1] Пользователь пишет заметку в Obsidian
        ↓
[2] ~/Documents/Творческий конвеер/1. Исчезающие заметки/
        ↓  human-layer routing / strategy session workflow
[3] Заметки сортируются внутри Obsidian-vault по рабочим зонам
    → ~/Documents/Творческий конвеер/2. Черновики/{Проект}/
...
Они не должны быть в публичном репо. Рабочий human-layer живёт локально
в `Творческий конвеер`, а governance-следы и work products materialize в `DS-strategy`.
...
**Важно:** текущий контракт больше не `Claude-only`.
Если `Codex` доступен, локальные агенты и канонические ритуалы должны проходить через `Codex-primary`.
`claude /login` нужен только для Claude-specific / fallback сценариев, а не как обязательный шаг для всего контура.
...
| 2026-02-22 | Ранний слой интеграции экзокортекса с Obsidian human-layer и VK-offee. |

...
### Базовая архитектурная версия

На 2026-04-27 в текущем финансовом РП принята базовая архитектура
`v0.5`:

- `v0.4 assets integrated` остаётся вычислительной базой;
- `v0.5` добавляет разделение контуров `business-finance` и
  `personal-finance`;
- `v0.3` не используется, потому что не включает активы в слой
  преобразования и ведёт к неполной финансовой модели.

Целевая переменная домена: `cash` (фактические деньги).

Первый фокус после фиксации `v0.5`: добить `business-finance core`, а не
разворачивать сразу `personal-finance`.
...
### Разделение контуров v0.5

`Business-finance` и `Personal-finance` не смешиваются.

`Business-finance` содержит:
...
Связь между контурами проходит только через явные трансферы:

- `transfer_out` — вывод денег из бизнеса;
- `transfer_in` — ввод денег в бизнес.

Запрещено учитывать личные расходы как расходы бизнеса и бизнес-активы как
личные расходы. Все движения между системами должны фиксироваться явно.

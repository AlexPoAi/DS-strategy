...
42. 2026-04-29: после восстановления extractor full-loop открыт `WP-128` на truthful verdict по human-layer tails. Сверка показала: `Доска выбора` не является upstream core Церена и не имеет отдельного Claude skill-route; её текущий warning вызван stale beacon (`updated: 2026-04-27`), а `Scout` живёт отдельным локальным слоем в `DS-agent-workspace/scout` и давно не обновлялся (март 2026), поэтому тоже требует отдельного truthful решения, а не маскировки под runtime-green.
43. 2026-04-29: `selection board stale` закрыт без runtime-fix'ов: штатный Obsidian-скрипт `selection_board_beacon.py` пересобрал `00-Сводка доски выбора.md`, после чего свежий `health-check` подтвердил `human-layer selection board beacon свежий` и итог `Среда исправна`. Это подтвердило, что `Доска выбора` была pure freshness-tail, а единственным оставшимся human-layer вопросом в `WP-128` остаётся governance-verdict по `Scout`.
44. 2026-04-29: `WP-128` закрыт с truthful verdict `Scout = local optional stale layer`. Это сняло последнюю неоднозначность между upstream-core и human-layer: `Scout` больше не трактуется как hidden mandatory runtime-service, а `Доска выбора` — как runtime defect.
45. 2026-04-29: `WP-127` закрыт как завершённый route-alignment slice. Финальный status-layer зелёный (`AGENTS-STATUS`, `health-check`), а локальные отклонения от Церена перечислены явно: absolute hooks, Obsidian/session-watcher bridge, `daily-report --refresh-status-artifacts`, human-layer beacon и optional `Scout`.
46. 2026-04-29: в `WP-129` materialized первый рабочий кусок по `Scout`: подтверждено отсутствие живого template/runtime runner, после чего в `DS-agent-workspace/scout/README.md` и `DS-agent-workspace/CLAUDE.md` зафиксирован правильный контракт `Scout` как review/input слоя для ядра Церена. Это переводит `Scout` из неопределённой stale-папки в явный data/review layer даже до восстановления отдельного запуска.

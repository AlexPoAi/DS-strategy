...
37. 2026-04-21: в `WP-97` закрыт factual audit по `ABC intake`: warehouse sync переведён с intake-подпапки на весь warehouse root, `.xlsx`/`pdf` гарантированно materialize локально, а `ABC анализ1/2.xlsx` возвращены из `Обработано` в `Новые документы`; после live-inspection подтверждено, что текущие файлы `ABC` сами по себе header-only и не содержат таблицы позиций, поэтому blocker перенесён с parser-layer на upstream export-quality.

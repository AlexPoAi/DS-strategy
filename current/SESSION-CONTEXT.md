...
## Где мы находимся
**Последнее обновление:** 2026-04-03 20:27
...
**Статус:** задача закрыта — WP: Починка Telegram-бота — убран дубль на Маке (plist удалён), исправлен note_command на VPS (порядок git: add→commit→pull→push), добавлен fallback при ошибке git. Бот перезапущен на VPS.

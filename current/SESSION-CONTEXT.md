...
## Где мы находимся
**Последнее обновление:** 2026-04-03 22:19
...
**Статус:** задача закрыта — WP-49: Добавлено правило 15 в CLAUDE.md — НЕ использовать --resume (баг кэша Claude Code v2.1.69+, расходы x10-20). Workaround: новая сессия каждый раз, контекст через SESSION-CONTEXT.md. Анализ экосистемы: агенты не затронуты (не используют resume).

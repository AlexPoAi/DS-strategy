...
## Где мы находимся
**Последнее обновление:** 2026-04-05 22:58
...
**Статус:** задача закрыта — Зафиксирована инженерная задача R-2: каскадный fallback моделей (Haiku→Sonnet, Opus запрещён). INBOX обновлён. Haiku недоступен на текущем API ключе — важный контекст для всех инженерных задач.

...
При закрытии дня итоговый ответ пользователю должен быть коротким truthful report в стиле:

```text
День закрыт — PASS / PASS WITH WARNINGS / FAIL
...
Правила verdict:
- `PASS` допустим только если проверки не оставили operational warnings.
- `PASS WITH WARNINGS` — день закрыт, но есть non-blocking хвосты вроде stale extractor, backup/path drift, не настроенного WakaTime или других зафиксированных yellow-сигналов.
- `FAIL` — есть blocking failure: незакоммиченные/непушнутые результаты, broken governance route, невозможность обновить DayPlan/WeekPlan/WP registry или критичный runtime defect.
- Нельзя сглаживать `yellow/red` в финальном тексте. Если система жёлтая, отчёт должен сказать это прямо и перечислить warnings.

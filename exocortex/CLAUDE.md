...
10. **РУССКОЕ НАЗВАНИЕ ФАЙЛА (БЛОКИРУЮЩЕЕ):** При создании ЛЮБОГО файла название ОБЯЗАТЕЛЬНО дублируется на русском языке в скобках. Формат: `ID-english-slug (Русское описание).md`. Примеры: `PARK.WP.022-letter-to-konstantin (Письмо архитектору Константину).md`, `KITCHEN.METHOD.001-ingredient-change (Замена ингредиента).md`. БЕЗ ИСКЛЮЧЕНИЙ. Нарушение = файл не создан.
11. **Отчёт сессии стратегирования (БЛОКИРУЮЩЕЕ):** При создании StrategyReport ОБЯЗАТЕЛЬНО включить ВСЕ задачи из `DS-strategy/inbox/INBOX-TASKS.md`. Разделить на две категории: (1) Действительные (можно разработать сейчас), (2) Ожидание (блокеры, внешние зависимости). Формат: карточки (не таблицы). Без этого отчёт неполный.
12. **Extractor лимит 5 captures (намеренный):** Ограничение на 5 captures за цикл — это намеренно, чтобы не раздувать план на неделю. Остальные задачи остаются в INBOX-TASKS и решаются по очереди. На новую неделю → переставляешь приоритеты вручную, выбирая из полного списка задач (не просто идёшь по очереди).

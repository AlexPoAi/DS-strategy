...
17. **Ритуал сначала, работа потом (БЛОКИРУЮЩЕЕ):** Перед любым новым рабочим циклом, инженерным slice или изменением файлов сначала определить/открыть РП и пройти ритуал. Короткий стоппер лежит в `exocortex/ritual-first.md`. Формула: `нет ритуала -> нет работы`.
18. **Инженерный pre-step "Церен -> эталон -> РП -> реализация" (БЛОКИРУЮЩЕЕ):** Для инженерных задач сначала смотреть уже реализованное решение у Церена, потом сверять его с нашим эталоном, и только после этого открывать/вести РП и вносить изменения. Это правило действует по всему инженерному контуру, а не только в текущей сессии.

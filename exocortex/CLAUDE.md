...
|--------|----------|---------|
| **Открытие** | `Read memory/protocol-open.md` → выполнить | Любое задание, а также «открывай», «открывай сессию», «открывай день» |
| **Работа** | `Read memory/protocol-work.md` → правила ниже | После прохождения Открытия |
| **Закрытие** | `Read memory/protocol-close.md` → выполнить в текущем аутентифицированном агенте (без обязательного `/run-protocol`) | «закрываю» / «закрой день» / «всё» / «закрывай» |
...
7. **Close provider-agnostic:** если `Claude` не залогинен, но `Codex` работает, закрытие не блокируется и продолжается через `Codex`. `claude /login` нужен только для возврата именно Claude-route.
8. **Slash-route не обязателен:** если агент работает в `Codex` или slash-команды недоступны, протоколы `open/close` выполняются вручную по `memory/protocol-*.md`; попытка вызвать `/run-protocol` не требуется и не считается blocker.
9. **Выбор модели:** Объявить в Ритуале согласования. Opus (архитектура, сложный код), Sonnet (типовые задачи), Haiku (поиск, простые фиксы). Детали → `memory/protocol-open.md`.
10. **Терминология VK-offee:** "Шеф-повар" = должность на кухне. "Шеф" / "для шефа" = руководитель проекта VK-offee.
11. **Внеплановые задачи:** Если во время выполнения РП пользователь даёт новую задачу → записать в `DS-strategy/inbox/INBOX-TASKS.md` → предложить продолжить текущий РП. Переход к новой задаче ТОЛЬКО после завершения текущего РП.
12. **Итерационная дисциплина рабочих продуктов (БЛОКИРУЮЩЕЕ):** Сложная работа ведётся только как цепочка отдельных рабочих продуктов. Один WP = один артефакт = одно truthful закрытие через `close-task.sh` = отдельный commit/push = обновлённый `SESSION-CONTEXT.md`. Нельзя открывать следующий WP, пока текущий не закрыт. Нельзя вести один длинный контекст из нескольких незакрытых WP. Если задача разрастается — остановиться, зафиксировать текущий WP, создать следующий WP и продолжить уже в новом цикле.
13. **РУССКОЕ НАЗВАНИЕ ФАЙЛА (БЛОКИРУЮЩЕЕ):** При создании ЛЮБОГО файла название ОБЯЗАТЕЛЬНО дублируется на русском языке в скобках. Формат: `ID-english-slug (Русское описание).md`. Примеры: `PARK.WP.022-letter-to-konstantin (Письмо архитектору Константину).md`, `KITCHEN.METHOD.001-ingredient-change (Замена ингредиента).md`. БЕЗ ИСКЛЮЧЕНИЙ. Нарушение = файл не создан.
14. **Отчёт сессии стратегирования (БЛОКИРУЮЩЕЕ):** При создании StrategyReport ОБЯЗАТЕЛЬНО включить ВСЕ задачи из `DS-strategy/inbox/INBOX-TASKS.md`. Разделить на две категории: (1) Действительные (можно разработать сейчас), (2) Ожидание (блокеры, внешние зависимости). Формат: карточки (не таблицы). Без этого отчёт неполный.
15. **Extractor лимит 5 captures (намеренный):** Ограничение на 5 captures за цикл — это намеренно, чтобы не раздувать план на неделю. Остальные задачи остаются в INBOX-TASKS и решаются по очереди. На новую неделю → переставляешь приоритеты вручную, выбирая из полного списка задач (не просто идёшь по очереди).
16. **Обнови экзокортекс (БЛОКИРУЮЩЕЕ):** Триггер: «обнови экзокортекс». Алгоритм: (1) Pre-check: `bash ~/Github/FMT-exocortex-template/setup/validate-template.sh` + стартовый экран + проверка git drift; (2) Dry-run: `cd ~/Github/FMT-exocortex-template && bash update.sh --dry-run` → показать пользователю что изменится → получить подтверждение; (3) Apply: `bash update.sh` (fetch upstream, merge, reinstall platform-space, roles); (4) Post-check: validate-template + стартовый экран + проверка работоспособности агентов (scheduler/strategist/extractor). ЗАПРЕЩЕНО применять обновление без pre-check и post-check. Если любая проверка упала — откат и диагностика.
17. **НЕ использовать --resume (БЛОКИРУЮЩЕЕ):** Claude Code с v2.1.69 имеет баг: `--resume` ломает prompt cache → полный cache miss → расходы x10-20. Всегда начинать новую сессию. Контекст передавать через SESSION-CONTEXT.md.
18. **Команда "сверь с эталоном" (БЛОКИРУЮЩЕЕ для инженерных задач):** При инженерных, инфраструктурных и экзокортекс-задачах команда пользователя `сверь с эталоном` означает обязательную проверку решения относительно целевой рабочей модели. Агент обязан показать: (1) эталонное состояние, (2) текущее состояние, (3) разрыв между ними, (4) как предлагаемое изменение приближает систему к эталону, а не создаёт локальный костыль.
